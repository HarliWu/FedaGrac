...
class a9a(Dataset):
    ...
    TRAIN_DOWNLOAD  = "https://www.csie.ntu.edu.tw/~cjlin/libsvmtools/datasets/binary/a9a"
    VAL_DOWNLOAD    = "https://www.csie.ntu.edu.tw/~cjlin/libsvmtools/datasets/binary/a9a.t"
    ...
    def _load_data(self):
        name = "a9a" if self.train else "a9a.t"
        path = os.path.join(self.root, name)
        f = open(path, 'r')
        self.data, self.targets = [], []
        for line in f.readlines():
            label_features = line[:-1].split(' ')
            label, data = 0 if label_features[0] == '-1' else 1, [0.0] * 123
...
def get_dataset(ranks:list, workers:list, batch_size:int, data_aug:bool=True, dataset_root='./dataset', **kwargs):
    trainset = a9a(root=dataset_root + '/a9a', train=True, download=True)
    testset = a9a(root=dataset_root + '/a9a', train=False, download=True)
...
def get_dataset_with_precat(ranks:list, workers:list, batch_size:int, test_required:bool=False, dataset_root='./dataset'):
    raise Exception("Not support in a9a dataset")
