...
def get_dataset(ranks:list, workers:list, batch_size:int, data_aug:bool=True, dataset_root='./dataset', **kwargs):
    if data_aug:
        trainset = CIFAR10(dataset_root + '/cifar10_data', train=True, download=True, transform=train_transform)
        testset = CIFAR10(dataset_root + '/cifar10_data', train=False, download=True, transform=test_transform)
    else: 
        trainset = CIFAR10(dataset_root + '/cifar10_data', train=True, download=True)
        testset = CIFAR10(dataset_root + '/cifar10_data', train=False, download=True)
...
def get_dataset_with_precat(ranks:list, workers:list, batch_size:int, test_required:bool=False, dataset_root='./dataset'):
    ...
    for rank in ranks:
        ...
        current_path = dataset_root + '/cifar10_data/{}_partitions/{}'.format(len(workers), idx)
...
def get_testdataset(batch_size: int, dataset_root='./dataset'):
    testset = CIFAR10(root=dataset_root + '/cifar10_data', train=False, download=True, transform=test_transform)
    testset = DataLoader(dataset=testset, batch_size=batch_size, shuffle=False)
    return testset

def get_testset_from_folder(batch_size:int, dataset_root='./dataset'):
    current_path = dataset_root + '/cifar10_data/testset'
...
if __name__ == "__main__":
    ...
    path = path + '/cifar10_data/{}_partitions'.format(num_workers)
