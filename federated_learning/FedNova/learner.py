...
def update_model(model, aggregated_model, args, weights, k_list, cpu, gpu):
    # all_param = model.state_dict()
    
    # send parameters variance to PS
...
def run(workers, size, model, args, data_ratio_pairs:dict, cpu, gpu):
    ...
    for t in range(args.T):
        ...
        aggregated_model_variance = [torch.zeros_like(param, device=gpu) for param in model.parameters()]
        k_list = []
        ...
        for idx, worker in enumerate(workers):
            if worker in part_list:
                ...
                k_list = k_list + [K] * np.count_nonzero(part_list==worker)
                ...
                for i, (my_param, global_param) in enumerate(zip(mymodel.parameters(), model.parameters())):
                    aggregated_model_variance[i] += (my_param.data - global_param.data) / K * weight

        loss_cpu = aggregated_loss.clone().detach().to(cpu)
        dist.gather(tensor=loss_cpu, dst=0)

        update_model(model, aggregated_model_variance, args, weights, k_list, cpu, gpu)
