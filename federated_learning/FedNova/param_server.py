...
def update_model(model, size, cpu, gpu):
    ...
    # receive the parameters variance from workers 
    for param in model.parameters():
        ...
        variance = torch.zeros_like(param.data, device=gpu)
        for w in range(size):
            # Suppose the model received from clients are well processed 
            variance = variance + gather_list[w].clone().detach().to(gpu)
    ...
    for idx, param in enumerate(model.parameters()):
        param.data = param.data + variance_list[idx] * avg_k
